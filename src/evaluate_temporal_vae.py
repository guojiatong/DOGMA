#!/usr/bin/env python3
"""
Evaluate a temporal VAE checkpoint on held-out pseudo-pose windows.
"""
...
def _build_model_from_config(config: TemporalVaeTrainingConfig, device: torch.device) -> TemporalPoseVAE:
    return TemporalPoseVAE(
        input_dim=36,
...
def load_temporal_vae_checkpoint(
    checkpoint_path: Path,
    *,
    device: torch.device,
) -> tuple[TemporalPoseVAE, TemporalVaeTrainingConfig, torch.Tensor, torch.Tensor, dict[str, Any]]:
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    config = TemporalVaeTrainingConfig(**checkpoint["config"])
    ...
    return model, config, pose_mean, pose_std, checkpoint
...
@torch.no_grad()
def evaluate_temporal_vae_checkpoint(
    *,
    checkpoint_path: Path,
    ...
    dataset = PseudoPoseWindowDataset.from_window_index_csv(
    ...
    global_metrics = evaluate_temporal_vae(
        model=model,
        dataloader=dataloader,
        device=device_resolved,
        beta=config.beta,
    ...
    for batch in dataloader:
        ...
        pose = batch["pose"]
        target = normalize_pose_tensor(pose, pose_mean, pose_std) if config.normalize_pose else pose
        reconstruction, _, _ = model(target)
        reconstruction_raw = denormalize_pose_tensor(reconstruction, pose_mean, pose_std) if config.normalize_pose else reconstruction
        ...
        for index in range(pose.shape[0]):
            ...
            metrics = compute_pose_recon_metrics(
                prediction=reconstruction_raw[index : index + 1],
                target=pose[index : index + 1],
            )
            row = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a temporal VAE checkpoint on pseudo-pose windows")
...
def main() -> None:
    args = parse_args()
    summary = evaluate_temporal_vae_checkpoint(
