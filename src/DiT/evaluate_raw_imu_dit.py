#!/usr/bin/env python3
"""
Evaluate a direct raw-IMU DiT checkpoint on held-out windows.
"""
...
from train_dit import _move_tensor_batch, build_sampling_generator, normalize_condition_tensor  # noqa: E402
from train_raw_imu_dit import (  # noqa: E402
    DEFAULT_CONFIG_DIR,
    RawImuLatentDiffusionWindowDataset,
    denormalize_target_tensor,
    load_raw_imu_dit_checkpoint,
    sample_feature_diffusion,
)
...
DEFAULT_EVAL_OUTPUT_DIR = DEFAULT_CONFIG_DIR / "eval_raw_imu_dit"
...
@torch.no_grad()
def evaluate_raw_imu_dit_checkpoint(
    *,
    ...
    for batch in progress:
        ...
        for index in batch_progress:
            ...
            target_norm_single = (target_feature_single - target_mean) / target_std
            target_shape = tuple(target_norm_single.shape)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate direct raw-IMU DiT on held-out windows")
...
def main() -> None:
    args = parse_args()
    summary = evaluate_raw_imu_dit_checkpoint(
