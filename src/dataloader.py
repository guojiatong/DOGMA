#!/usr/bin/env python3
"""
Self-contained dataloader utilities for DOGMA latent diffusion training.

This file is intentionally standalone so it can be shared together with:

- `features_20hz/`
- `pseudo_pose_20hz/`
- `manifests_8s160f_remote/vae_window_index_{train,val,test}.csv`

The canonical latent diffusion task is:

- condition: `past 6s raw IMU` at 20Hz -> `120 x 10 x 13`
- target: `8s combined pseudo-pose window` at 20Hz -> `160 x 36`

The shared manifest stores absolute paths from the original machine. On a new
machine you will usually want to pass `data_root=...` so paths are rebuilt as:

- `<data_root>/features_20hz/<participant>/segment_<segment_id>.npz`
- `<data_root>/pseudo_pose_20hz/<participant>/segment_<segment_id>.npz`
"""
...
class LatentDiffusionWindowDataset(Dataset):
    """
    Dataset for DOGMA latent diffusion.

    Important: the 8s/160f manifest is reused in a slightly unusual way.
    `record.start_frame` is treated as the future-start anchor, so each sample is:

    - past context: `[start_frame - past_frames, start_frame)`
    - future target: `[start_frame, start_frame + future_window_frames)`
    - combined pose window: `[start_frame - past_frames, start_frame + future_window_frames)`
    """
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if condition.shape != (self.past_frames, CONDITION_DIM):
            raise ValueError(f"condition shape mismatch: expected {(self.past_frames, CONDITION_DIM)}, got {condition.shape}")
        ...
        if target_pose.shape[0] != self.past_frames + self.future_window_frames:
            raise ValueError(
                "target_pose frame count mismatch: expected "
                f"{self.past_frames + self.future_window_frames}, got {target_pose.shape[0]}"
            )
...
def build_latent_diffusion_dataloader(
    *,
    window_index_csv: str | Path,
    batch_size: int,
    shuffle: bool,
    ...
    path_prefix_map: Mapping[str, str] | Sequence[tuple[str, str]] | None = None,
    validate_paths: bool = False,
) -> tuple[LatentDiffusionWindowDataset, DataLoader]:
...
__all__ = [
    ...
    "build_latent_diffusion_dataloader",
